...
data_dir = "/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/row"
processed_dir = "/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/processed"
big_processed_dir = "/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/processed"
processed_img_dir = "/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/processed/crops"
...
def load_data():
    ...
    for k, f in enumerate(file_list):
        ...
        for i, l in enumerate(f.readlines()):
            ...
            if (i + 1) % 3 == 0:
                l_original = clean_str_sst(str(l, "utf-8"))
...
def split_EANN_simple(df_data):
    ...
    """
    id_test = pickle.load(open("/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/row/test_id.pickle", 'rb'))
    id_train = pickle.load(open("/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/row/train_id.pickle", 'rb'))
    id_valid = pickle.load(open("/sda/home/temp/anwenxin/c3n_origin/code/Weibo16/row/validate_id.pickle", 'rb'))
...
def save_text_input(df):
    ...
    np.save(processed_dir + '/n_words.npy', n_token_dic)
    np.save(processed_dir + '/word_clipinputs.npy', text_input_dic)
    

def split_origin_post_to_sentences(text):
    """
    将原始文本分割为句子，返回句子列表。
    支持中英文标点。
    """
...
def generate_data_text_df():
    """
    读取 row_data_df.npy，分割 original_post 为 sentences，生成 data_text_df.npy
    """
    input_path = processed_dir + "/row_data_df.npy"
    output_path = processed_dir + "/data_text_df.npy"
    # 读取数据
    df = np.load(input_path, allow_pickle=True)
    columns = ['post_id', 'image_id', 'original_post', 'label']
    df = pd.DataFrame(df, columns=columns)
    # 分割句子
    df['sentences'] = df['original_post'].apply(split_origin_post_to_sentences)
    # 保存
    np.save(output_path, df.values)
    print(f"Saved data_text_df.npy with sentences to {output_path}")
