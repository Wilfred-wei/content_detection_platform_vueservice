...
def clip_image_preprocess(df, preprocess):
    ...
    error_crop_count = 0
    for i, path in enumerate(tqdm(os.listdir(processed_img_dir))):
        crop_list = []
...
if __name__ == '__main__':
    ...
    model, preprocess = load_from_name("ViT-B-16", device=device, download_root='./pretrained_models/cn-clip/')
    model.float()
