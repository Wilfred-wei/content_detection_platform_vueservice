...
import type { SafeConfig } from "./config.js";
...
export function assertSafeWorkerResult(value: unknown): ModelDetectionResult & { requestId: string } {
  if (!value || typeof value !== "object") throw new Error("SAFE_MALFORMED_RESPONSE:not_an_object");
  const result = value as Record<string, unknown>;
  if (result.protocolVersion !== MODEL_DETECTOR_PROTOCOL_VERSION) throw new Error("SAFE_MALFORMED_RESPONSE:protocol_version");
  if (typeof result.requestId !== "string" || !result.requestId) throw new Error("SAFE_MALFORMED_RESPONSE:request_id");
  if (result.detectorId !== "safe-wavelet-resnet") throw new Error("SAFE_MALFORMED_RESPONSE:detector_id");
  if (!( ["detected", "not_detected", "unavailable", "unsupported_format", "error"] as unknown[]).includes(result.outcome)) {
    throw new Error("SAFE_MALFORMED_RESPONSE:outcome");
  }
  if (result.score !== null && (typeof result.score !== "number" || !Number.isFinite(result.score) || result.score < 0 || result.score > 1)) {
    throw new Error("SAFE_MALFORMED_RESPONSE:score");
  }
  if (result.threshold !== null && (typeof result.threshold !== "number" || !Number.isFinite(result.threshold) || result.threshold < 0 || result.threshold > 1)) {
    throw new Error("SAFE_MALFORMED_RESPONSE:threshold");
  }
  if (typeof result.latencyMs !== "number" || !Number.isFinite(result.latencyMs) || result.latencyMs < 0) {
    throw new Error("SAFE_MALFORMED_RESPONSE:latency");
  }
  if (typeof result.detectorVersion !== "string" || typeof result.preprocessingId !== "string"
    || !result.detectorVersion.startsWith("SAFE-official-kdd2025-")
    || result.preprocessingId !== "safe-center-crop256-totensor-v1"
    || !["official_threshold_unverified_for_deployment", "deployment_calibrated"].includes(String(result.calibrationStatus))) {
    throw new Error("SAFE_MALFORMED_RESPONSE:identity");
  }
  if (!(result.predictedClass === null || result.predictedClass === "ai_generated" || result.predictedClass === "non_ai")) {
    throw new Error("SAFE_MALFORMED_RESPONSE:predicted_class");
  }
  if (result.outcome === "detected" || result.outcome === "not_detected") {
    if (typeof result.score !== "number" || typeof result.threshold !== "number") {
      throw new Error("SAFE_MALFORMED_RESPONSE:missing_score");
    }
    const expectedClass = result.score > result.threshold ? "ai_generated" : "non_ai";
    const expectedOutcome = expectedClass === "ai_generated" ? "detected" : "not_detected";
    if (result.predictedClass !== expectedClass || result.outcome !== expectedOutcome) {
      throw new Error("SAFE_MALFORMED_RESPONSE:score_direction");
    }
  }
  if (!result.diagnostics || typeof result.diagnostics !== "object" || Array.isArray(result.diagnostics)) {
    throw new Error("SAFE_MALFORMED_RESPONSE:diagnostics");
  }
  return result as unknown as ModelDetectionResult & { requestId: string };
}

export class SafeModelDetector implements ModelDetector {
  readonly id = "safe-wavelet-resnet";
  ...
  constructor(private readonly config: SafeConfig) {
  ...
  detect(asset: MediaAsset): Promise<ModelDetectionResult> {
    if (!this.enabled) throw new Error("SAFE_DISABLED");
    if (this.queuedRequests >= this.config.maxQueue) throw new Error(`SAFE_QUEUE_FULL:${this.config.maxQueue}`);
  ...
  async detectBatch(assets: readonly MediaAsset[]): Promise<readonly ModelDetectionResult[]> {
    ...
    if (assets.length > 32) throw new Error("SAFE_BATCH_TOO_LARGE:32");
    if (this.queuedRequests + assets.length > this.config.maxQueue) throw new Error(`SAFE_QUEUE_FULL:${this.config.maxQueue}`);
  ...
  close(): void {
    ...
    this.rejectPending(new Error("SAFE_WORKER_CLOSED"));
  }
  ...
  private async detectAdmitted(asset: MediaAsset): Promise<ModelDetectionResult> {
    ...
    const requestId = `safe-${process.pid || "starting"}-${++this.requestSequence}`;
    const payload = {
    ...
    return new Promise<ModelDetectionResult>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`SAFE_TIMEOUT:${this.config.timeoutMs}`));
        this.restartWorker();
      ...
      process.stdin.write(`${JSON.stringify(payload)}\n`, (error) => {
        ...
        this.pending.delete(requestId);
        pending.reject(new Error(`SAFE_WORKER_WRITE_FAILED:${error.message}`));
      });
    });
  }
  ...
  private async detectBatchAdmitted(assets: readonly MediaAsset[]): Promise<readonly ModelDetectionResult[]> {
    ...
    const requests = supported.map(({ asset }) => ({
      protocolVersion: MODEL_DETECTOR_PROTOCOL_VERSION,
      requestId: `safe-${process.pid || "starting"}-${++this.requestSequence}`,
      imagePath: asset.storedPath,
      mimeType: asset.mimeType,
      assetSha256: asset.sha256,
    }));
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        process.stdin.write(`${JSON.stringify({ protocolVersion: MODEL_DETECTOR_PROTOCOL_VERSION, requests })}\n`, (error) => {
          if (error) reject(new Error(`SAFE_WORKER_WRITE_FAILED:${error.message}`));
          else resolve();
        });
      });
    } catch (error) {
  ...
  private ensureProcess(): ChildProcessWithoutNullStreams {
    ...
    const child = spawn(this.config.uvCommand, [
      "run", "--project", this.config.workerProjectDir, "--frozen", "--offline", "--no-sync", "safe-worker",
      "--source-dir", this.config.sourceDir,
      "--source-revision", this.config.sourceRevision,
      "--source-sha256", this.config.sourceSha256,
    ...
    child.once("error", (error) => {
      this.rejectPending(new Error(`SAFE_WORKER_START_FAILED:${error.message}`));
    });
    child.once("exit", (code, signal) => {
      if (this.process === child) this.process = undefined;
      this.rejectPending(new Error(`SAFE_WORKER_EXITED:${code ?? signal ?? "unknown"}:${this.stderr}`));
    });
    return child;
  }
  ...
  private handleLine(line: string): void {
    ...
    try {
      parsed = assertSafeWorkerResult(JSON.parse(line));
      if (parsed.checkpointSha256 !== this.config.checkpointSha256
        || parsed.diagnostics.sourceRevision !== this.config.sourceRevision
        || parsed.diagnostics.sourceSha256 !== this.config.sourceSha256) {
        throw new Error("SAFE_MALFORMED_RESPONSE:artifact_identity");
      }
    } catch (error) {
      this.rejectPending(error instanceof Error ? error : new Error("SAFE_MALFORMED_RESPONSE"));
      this.restartWorker();
      return;
    }
  ...
  private localResult(outcome: ModelDetectionResult["outcome"], diagnostics: ModelDetectionResult["diagnostics"]): ModelDetectionResult {
    return {
      protocolVersion: MODEL_DETECTOR_PROTOCOL_VERSION,
      detectorId: this.id,
      detectorVersion: `SAFE-official-kdd2025-${this.config.sourceRevision.slice(0, 8)}`,
      outcome,
      score: null,
      threshold: null,
      predictedClass: null,
