            ...
            <div v-if="currentJob || isProcessing" class="job-status">
              ...
              <div class="job-actions mt-4">
                <div class="d-grid gap-2 d-md-flex">
                  <div v-if="currentJob?.status === 'completed'" class="btn-group position-relative">
                    <button @click="downloadResults" class="btn btn-primary">
                      <i class="fas fa-download me-2"></i>下载结果
                    </button>
                    <button ref="dropdownButton" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" @click="toggleExportDropdown">
                    ...
                    <ul v-if="showExportDropdown" class="dropdown-menu show"
                      ...
                      <li><a class="dropdown-item" href="#" @click.prevent="exportBatchResults('csv'); showExportDropdown = false">
                        <i class="fas fa-file-csv me-2"></i>导出CSV
                      </a></li>
                      <li><a class="dropdown-item" href="#" @click.prevent="exportBatchResults('json'); showExportDropdown = false">
                  ...
                  <button v-if="currentJob && currentJob?.status === 'completed'"
                          @click="showAllResultsModal"
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const showAllResultsModal = () => {
      // 显示第一个结果的详情，如果没有结果则不操作
      if (currentJob.value?.results && currentJob.value.results.length > 0) {
        currentResultIndex.value = 0
        currentResult.value = currentJob.value.results[0]
        showResultModal.value = true
      }
    }
    ...
    const downloadCurrentImage = async () => {
      ...
      // 使用 normalizeServiceUrl 处理 URL，确保包含 /M3/ 前缀
    ...
    const toggleExportDropdown = () => {
      ...
      // 计算下拉菜单位置
      if (showExportDropdown.value && dropdownButton.value) {
    ...
    // 将后端返回的 URL 统一转换为代理路径，避免跨域/外网不可达
    const normalizeServiceUrl = (url?: string) => {
      if (!url) return url as any
      try {
        // 如果已经是完整的 http/https URL，且不是 localhost
